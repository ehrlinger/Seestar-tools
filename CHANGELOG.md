...
### Fixed
- **The pipeline no longer descends into `_trash/` (or `scripts/`).** Target discovery used a recursive `rglob`, so `rename`, `organize`, `sort` (`fits_dirs`), `count`, and `batch_stack` would all reach into a `_trash/` folder in the archive and process whatever was set aside there — e.g. reporting "would rename 4 folder(s)" for empty stray folders living under `_trash/strays/`, and (if real subs were ever trashed) renaming/sorting/counting them. Discovery now skips any path under an excluded directory name (`_trash`, `scripts`) via a new shared `seestar_common.is_in_excluded()` helper. Covered by 9 unit tests.
