...
- **Unit test suite** (`tests/test_parsing.py`, 74 tests using stdlib `unittest` — no install required). Covers `canonical_target_name`, `is_processed`, `safe_dest`, `group_by_target`, `_EXPTIME_RE`, `format_duration`, `normalize`, `canonical_folder_name`, `deduplicate`, `new_name`, `STACKED_RE`, plus `sort_by_exptime.exptime_label` / `EXPTIME_DIR_RE` / `is_already_sorted` / `find_target_dirs`, `count_subs.resolve_inventory_path`, `batch_stack.find_stack_units`, and `purge_siril_cruft.find_target_dirs`.
