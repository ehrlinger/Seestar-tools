...
### Fixed
- **The pipeline no longer treats anything under `_trash/` (or `scripts/`) as a target.** Target discovery used a recursive `rglob`, so `rename`, `organize`, `sort` (`fits_dirs`), `count`, and `batch_stack` would pick up folders inside a `_trash/` directory in the archive and process whatever was set aside there — e.g. reporting "would rename 4 folder(s)" for empty stray folders living under `_trash/strays/`, and (if real subs were ever trashed) renaming/sorting/counting them. Discovery still walks the tree but now **filters out** any path under an excluded directory name (`_trash`, `scripts`, case-insensitively) via a new shared `seestar_common.is_in_excluded()` helper. Covered by 10 unit tests.
- **`batch_stack.py` now sees the canonical `<exptime>s/lights/` layout.** Discovery, "has lights", and stale-stack detection previously assumed a flat `<target>_sub/lights/`, so after the pipeline's Step 3b exposure-sort moved subs into `<target>_sub/<exp>s/lights/`, batch stacking silently skipped every sorted target as `[no lights]` — and the documented per-exposure invocation found zero folders. Discovery now finds **stack units** (any directory holding a `lights/` of raw FITS), which matches both the legacy flat layout and the canonical exposure-sorted one; each `<exp>s/` folder is treated as its own unit so 10 s and 20 s subs stack separately. Works whether pointed at the archive root, a `<target>_sub`, or an individual `<target>_sub/<exp>s`. As part of this, `lights/` counting now skips macOS `._` resource-fork files so sub counts (and staleness comparisons) aren't inflated.
